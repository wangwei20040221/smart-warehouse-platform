...
public class WmsStockInOrdersController {
	...
	public Result<String> add(@RequestBody WmsStockInOrdersPage wmsStockInOrdersPage) {
		...
		List<WmsStockInOrderItems> items = wmsStockInOrdersPage.getWmsStockInOrderItemsList();
		if (items != null && !items.isEmpty()) {
			int totalExpectedQuantity = 0;
			for (WmsStockInOrderItems item : items) {
				item.setOrderId(wmsStockInOrders.getId());
				item.setStatus(WarehouseDictEnum.INBOUND_DETAIL_INITIAL.getCode());
				wmsStockInOrderItemsService.save(item);
				totalExpectedQuantity += item.getExpectedQuantity() != null ? item.getExpectedQuantity() : 0;
			}
			wmsStockInOrders.setTotalExpectedQuantity(totalExpectedQuantity);
			wmsStockInOrdersService.updateById(wmsStockInOrders);
		}
